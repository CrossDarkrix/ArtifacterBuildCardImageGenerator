...
from ArtifacterFont import ArtifactFont
from collections import Counter
from decimal import Decimal, ROUND_HALF_UP
from ImageBytes import *
from io import BytesIO
from PIL import Image, ImageFont, ImageFile, ImageDraw, ImageEnhance
...
class Artifacter(object):
    def __init__(self, uid):
        ...
        self.artifact_assets = ArtifactAssets
    ...
    def start_init(self):
        self.locale_jp = concurrent.futures.ThreadPoolExecutor().submit(json.loads, self.locale()).result()['ja']  # データの表記名データをEnka.Network公式Githubから取得
        self.characters_json = concurrent.futures.ThreadPoolExecutor().submit(json.loads, self.character_json()).result()  # キャラクターデータをEnka.Network公式Githubから取得
        self.namecards = concurrent.futures.ThreadPoolExecutor().submit(json.loads, self.namecard_json()).result()  # キャラクターのネームカードデータをEnkaNetwork公式Githubから取得
        try:
            self.player_data = concurrent.futures.ThreadPoolExecutor().submit(json.loads, urllib.request.urlopen(urllib.request.Request('https://enka.network/api/uid/{}'.format(self.uid), headers={'User-Agent': UsrAgn})).read().decode(errors='ignore')).result()
        except:
            self.player_data = {}
    ...
    def character_json(self):
        return urllib.request.urlopen(urllib.request.Request('https://raw.githubusercontent.com/EnkaNetwork/API-docs/master/store/characters.json', headers={'User-Agent': UsrAgn})).read().decode(errors='ignore')
    
    def namecard_json(self):
        return urllib.request.urlopen(urllib.request.Request('https://raw.githubusercontent.com/EnkaNetwork/API-docs/master/store/namecards.json', headers={'User-Agent': UsrAgn})).read().decode(errors='ignore')
    
    def option_calculation(self, data: dict):  # 聖遺物の詳細を計算
        dup = json.loads(self.mainoption())  # 聖遺物のメインオプション
        mapping = json.loads(self.suboption())  # 聖遺物のサブオプション
    ...
    def full_playerinfo(self, avateriD):  # プレイヤーデータの完全版を取得
        ...
        if 'avatarInfoList' in self.player_data:  # 取得したデータから「avaterInfoList」がある場合以下を実行
            avatar_data = list(filter(lambda item: item.get('avatarId') == int(avateriD), self.player_data['avatarInfoList']))[0]
        return avatar_data
    ...
    def create_buildcard(self, avaterinfo, score_state):
        ...
        character_element_buff_json = {'30': '物理ダメージ', '40': '炎元素ダメージ', '41': '雷元素ダメージ', '42': '水元素ダメージ', '43': '草元素ダメージ', '44': '風元素ダメージ', '45': '岩元素ダメージ', '46': '氷元素ダメージ'}
                ...
                for sk in list(map(int, avaterinfo['skillLevelMap'].keys())):
                    try:
                        if PMap['{}'.format(sk)] in list(map(int, avaterinfo['proudSkillExtraLevelMap'].keys())):
                            avaterinfo['skillLevelMap']['{}'.format(sk)] += avaterinfo['proudSkillExtraLevelMap'][PMap['{}'.format(sk)]]
                    except:
                        pass
            except:
                pass
        ...
        try:
            weapon_sub_name = self.locale_jp[list(filter(lambda item: item['appendPropId'] != 'FIGHT_PROP_BASE_ATTACK', weapon['flat']['weaponStats']))[0]['appendPropId']]
        except:
            weapon_sub_name = None
        try:
            weapon_sub_value = list(filter(lambda item: item['appendPropId'] != 'FIGHT_PROP_BASE_ATTACK', weapon['flat']['weaponStats']))[0]['statValue']
        except:
            weapon_sub_value = '1'
        weapon_icon = '{}.png'.format(weapon['flat']['icon'])
        # ここまで武器データ
        # ここから聖遺物データ
        artifact_state = {'HP': 'HPパーセンテージ', '攻撃': '攻撃パーセンテージ', '防御': '防御パーセンテージ', 'チャージ': '元素チャージ効率', '元素熟知': '元素熟知'}
        artifact_total_score = 0
        artifacts_score = {'total': 0.0, 'state': score_state, 'flower': 0.0, 'wing': 0.0, 'clock': 0.0, 'cup': 0.0, 'crown': 0.0}
        artifact_list = list(filter(lambda item: item.get('reliquary'), avaterinfo['equipList']))
        artifact_type_name = {'EQUIP_BRACER': 'flower', 'EQUIP_NECKLACE': 'wing', 'EQUIP_SHOES': 'clock', 'EQUIP_RING': 'cup', 'EQUIP_DRESS': 'crown'}
        artifacts = {'flower': {'type': '', 'icon': '', 'level': 1, 'rarelity': 1, 'mainoption': {'optionName': '', 'value': 0}, 'suboption': []}, 'wing': {'type': '', 'icon': '', 'level': 1, 'rarelity': 1, 'mainoption': {'optionName': '', 'value': 0}, 'suboption': []}, 'clock': {'type': '', 'icon': '', 'level': 1, 'rarelity': 1, 'mainoption': {'optionName': '', 'value': 0}, 'suboption': []}, 'cup': {'type': '', 'icon': '', 'level': 1, 'rarelity': 1, 'mainoption': {'optionName': '', 'value': 0}, 'suboption': []}, 'crown': {'type': '', 'icon': '', 'level': 1, 'rarelity': 1, 'mainoption': {'optionName': '', 'value': 0}, 'suboption': []}}
        for _Artifact in artifact_list:
            ...
            if not artifact == None:  # 聖遺物情報があるかないか。あるなら以下実行
                ...
                for i, s in enumerate(_Artifact['flat']['reliquarySubstats']):
                    ...
                    artifacts_score[artifact_type_name[_Artifact['flat']['equipType']]] = Decimal(artifactscore).quantize(Decimal('0.1'), rounding=ROUND_HALF_UP)
                    # ここまで聖遺物のサブオプションの設定
                artifact_total_score += Decimal(artifactscore)
        # ここまで聖遺物のスコアを計算
        artifacts_score['total'] = '{:.1f}'.format(Decimal(artifact_total_score).quantize(Decimal('0.1'), rounding=ROUND_HALF_UP))  # 各聖遺物を合わせたスコア
        ...
        with self.ArtifactFont() as Font:  # フォントファイルを一時フォルダへ展開後、フォントファイルを読み込み
            ...
            character_paste = Image.alpha_composite(character_paste, character_status_blur_paste)  # キャラクターの下に表示される好感度などのステータスの背景をブラーをかけてキャラクター画像と合成
            BaseImage = Image.alpha_composite(BaseImage, character_paste)  # ベース画像とキャラクター画像を合成
            # 武器画像の編集
            weapon_image = Image.open(BytesIO(urllib.request.urlopen(urllib.request.Request('https://enka.network/ui/{}'.format(weapon_icon), headers={'User-Agent': UsrAgn})).read())).convert('RGBA').resize((128, 128))  # EnkaNetworkから武器の画像データを取得
            ...
            try:  # 武器のレアリティ画像をRBGAに変換して読み込む
                weapon_rarelity_image = Image.open(BytesIO(self.artifact_rarelity('{}'.format(weapon_rarelity)))).convert('RGBA')
            ...
            weapon_rarelity_image = weapon_rarelity_image.resize((int(weapon_rarelity_image.width * 0.97), int(weapon_rarelity_image.height * 0.97)))
            ...
            character_talent_base = character_talent_base.resize((int(character_talent_base.width / 1.5), int(character_talent_base.height / 1.5)))  # 天賦のベース画像をリサイズ
            for i, t in enumerate(character_skill_icons):  # 天賦ベース画像とスキルアイコンとの合成
                _talent_paste = Image.new('RGBA', character_talent_base.size, (255, 255, 255, 0))  # 天賦のベース画像に合わせてベース画像を作成
                _talent = Image.open(BytesIO(urllib.request.urlopen(urllib.request.Request('https://enka.network/ui/{}'.format(t), headers={'User-Agent': UsrAgn})).read())).resize((50, 50)).convert('RGBA')  # EnkaNetworkから各種天賦画像を取得
                _talent_mask = _talent.copy()  # 取得した天賦画像でマスクの作成
                _talent_paste.paste(_talent, (_talent_paste.width // 2 - 25, _talent_paste.height // 2 - 25), mask=_talent_mask)  # 新規ベース画像に取得した天賦画像をリサイズして貼り付け
            ...
            character_constellations_base = Image.open(BytesIO(self.artifact_constellation(character_element))).resize((90, 90)).convert('RGBA')  # キャラクターの凸数のベース画像
            character_constellations_lock_base = Image.open(BytesIO(self.artifact_constellation('{}LOCK'.format(character_element)))).convert('RGBA')  # キャラクターの凸数のロックされた画像のベース
            ...
            for c, t in enumerate(character_const_icons):
                if c > int(character_const):  # キャラクターの凸数が「c」より小さいなら以下を実行
                    character_constellations_paste.paste(character_constellations_lock_base, (1016, 235 + (c + 1) * 93), mask=character_constellations_lock_base_mask)  # キャラクターのロックされた凸数をベースへ貼り付け
                else:  # キャラクターの凸数が「c」より大きいなら以下を実行
                    _character_const = Image.open(BytesIO(urllib.request.urlopen(urllib.request.Request('https://enka.network/ui/{}'.format(t), headers={'User-Agent': UsrAgn})).read())).convert('RGBA').resize((45, 45))  # キャラクターの凸数に応じて凸の画像を取得
                    _character_const_paste = Image.new('RGBA', character_constellations_base.size, (255, 255, 255, 0))  # 凸数のベース画像をベースに新しい画像を作成
                    _character_const_mask = _character_const.copy()  # 凸数のマスクを作成
                    _character_const_paste.paste(_character_const, (int(_character_const_paste.width / 2) - 25, int(_character_const_paste.height / 2) - 23), mask=_character_const_mask)  # キャラクターの凸数をマスクを指定しつつ合成
                    _character_const_object = Image.alpha_composite(character_constellations_base, _character_const_paste)  # 凸数のベース画像と上の合成した凸数の画像とさらに合成
                    character_constellations_paste.paste(_character_const_object, (1010, 235 + (c + 1) * 93))  # 凸数を合成したデータを指定の位置に指定した間隔で貼り付け
            ...
            def genbasetext(state):
                ...
                return f"+{format(plusv, ',')}", f"{format(basev, ',')}", DR.textlength(f"+{format(plusv, ',')}", font=artifact_font(12)), DR.textlength(f"{format(basev, ',')}", font=artifact_font(12))
            ...
            if weapon_sub_name != None:  # 武器のサブ名が有効なら以下を実行
                w_baseAttack = Image.open(BytesIO(self.artifact_emotes(weapon_sub_name))).convert('RGBA').resize((23, 23))
                w_baseAttack_mask = w_baseAttack.copy()
            ...
            score_conversion_len = DR.textlength('{}換算'.format(artifacts_score["state"]), font=artifact_font(24))  # 換算表記の文字数を計算
            DR.text((916 - score_conversion_len, 840), '{}換算'.format(artifacts_score["state"]), font=artifact_font(24))  # 聖遺物を評価する方式を表記
            # トータルスコアのランク決め
            ...
            for i, part in enumerate(['flower', "wing", "clock", "cup", "crown"]):  # 聖遺物の画像を合成していく
                ...
                if artifact_icons == '':
                    ...
                    artifact_preview = Image.open(BytesIO(urllib.request.urlopen(urllib.request.Request('https://enka.network/ui/{}'.format(artifact_icons), headers={'User-Agent': UsrAgn})).read())).convert('RGBA').resize((256, 256))  # 聖遺物の画像をEnakNetworkから取得
                    ...
                    if part in ['flower']:  # 聖遺物の花をベースに貼り付け
                        concurrent.futures.ThreadPoolExecutor(os.cpu_count() * 99999).submit(artifact_preview_paste.paste, artifact_preview, (50 + 420 * i, 901), mask=artifact_preview_mask)
                    elif part in ['wing']:  # 聖遺物の羽をベースに貼り付け
                        concurrent.futures.ThreadPoolExecutor(os.cpu_count() * 99999).submit(artifact_preview_paste.paste, artifact_preview, (-12 + 420 * i, 906), mask=artifact_preview_mask)
                    elif part in ['cup']:  # 聖遺物の盃をベースに貼り付け
                        concurrent.futures.ThreadPoolExecutor(os.cpu_count() * 99999).submit(artifact_preview_paste.paste, artifact_preview, (-105 + 420 * i, 896), mask=artifact_preview_mask)
                    elif part in ['crown']:  # 聖遺物の冠をベースに貼り付け
                        concurrent.futures.ThreadPoolExecutor(os.cpu_count() * 99999).submit(artifact_preview_paste.paste, artifact_preview, (-140 + 420 * i, 901), mask=artifact_preview_mask)
                    else:
                        concurrent.futures.ThreadPoolExecutor(os.cpu_count() * 99999).submit(artifact_preview_paste.paste, artifact_preview, (-35 + 420 * i, 916), mask=artifact_preview_mask)
                    ...
                    try:
                        MainOptionLen = DR.textlength(OptionMap[MainOption] or MainOption, font=artifact_font(29))  # 聖遺物のメインオプションの文字長を計算
                    except:
                        MainOptionLen = DR.textlength(MainOption, font=artifact_font(29))  # 聖遺物のメインオプションの文字長を計算
                    try:
                        DR.text((375 + i * 373 - int(MainOptionLen), 941), OptionMap[MainOption] or MainOption, font=artifact_font(29))  # メインオプションを書き込み
                    except:
                        DR.text((375 + i * 373 - int(MainOptionLen), 941), MainOption, font=artifact_font(29))  # メインオプションを書き込み
                    MainOption_icon = Image.open(BytesIO(self.artifact_emotes(MainOption))).convert('RGBA').resize((35, 35))  # メインオプションのアイコンを取得
                    MainOption_icon_mask = MainOption_icon.copy()  # メインオプションのアイコンのマスクを作成
                    BaseImage.paste(MainOption_icon, (335 + i * 373 - int(MainOptionLen), 941), mask=MainOption_icon_mask)
                    ...
                    if MainOption in Disper:
                        ...
                        DR.text((375 + i * 373 - MainOptionValueSize, 971), '{}%'.format(float(MainOptionValue)), font=artifact_font(49))
                    else:
                    ...
                    for a, arts in enumerate(artifact_details['suboption']):
                        ...
                        if artifact_suboption in ['HP', '攻撃力', '防御力']:
                            try:
                                DR.text((79 + 373 * i, 1109 + 50 * a), OptionMap[artifact_suboption] or artifact_suboption, font=artifact_font(25), fill=(255, 255, 255, 190))
                            except:
                                DR.text((79 + 373 * i, 1109 + 50 * a), artifact_suboption, font=artifact_font(25), fill=(255, 255, 255, 190))
                        else:
                            try:
                                DR.text((79 + 373 * i, 1109 + 50 * a), OptionMap[artifact_suboption] or artifact_suboption, font=artifact_font(25))
                            except:
                                DR.text((79 + 373 * i, 1109 + 50 * a), artifact_suboption, font=artifact_font(25))
                        artifact_suboption_icon = Image.open(BytesIO(self.artifact_emotes(artifact_suboption))).convert('RGBA').resize((30, 30))
                        artifact_suboption_icon_mask = artifact_suboption_icon.copy()
                        BaseImage.paste(artifact_suboption_icon, (44 + 373 * i, 1109 + 50 * a), mask=artifact_suboption_icon_mask)
                        if artifact_suboption in Disper:
                            artifact_suboption_size = DR.textlength('{}%'.format(float(artifact_subvalue)), artifact_font(25))
                            DR.text((375 + i * 373 - artifact_suboption_size, 1109 + 50 * a), '{}%'.format(float(artifact_subvalue)), font=artifact_font(25))
                        else:
                            artifact_suboption_size = DR.textlength(format(artifact_subvalue, ","), artifact_font(25))
                            if artifact_suboption in ['防御力', '攻撃力', 'HP']:
                                DR.text((375 + i * 373 - artifact_suboption_size, 1109 + 50 * a), format(artifact_subvalue, ","), font=artifact_font(25), fill=(255, 255, 255, 190))
                            else:
                                DR.text((375 + i * 373 - artifact_suboption_size, 1109 + 50 * a), format(artifact_subvalue, ","), font=artifact_font(25), fill=(255, 255, 255))
                        if artifact_details['level'] == 20 and artifact_details['rarelity'] == 5:  # 聖遺物のレベルが２０でかつレアリティが５なら以下を実行
                            artifact_stretch = DR.textlength("+".join(map(str, PSB[a])), font=artifact_font(11))
                            DR.text((375 + i * 373 - artifact_stretch, 1142 + 50 * a), "+".join(map(str, PSB[a])), fill=(255, 255, 255, 160), font=artifact_font(11))
                    ...
                    DR.text((211 + i * 373 - artifactsScoreLen, 1358), 'Score', font=artifact_font(27), fill=(160, 160, 160))
                    PRefer = {'total': {'SS': 220, 'S': 200, 'A': 180}, 'flower': {'SS': 50, 'S': 45, 'A': 40}, 'wing': {'SS': 50, 'S': 45, 'A': 40}, 'clock': {'SS': 45, 'S': 40, 'A': 35}, 'cup': {'SS': 45, 'S': 40, 'A': 37}, 'crown': {'SS': 40, 'S': 35, 'A': 30}}
                    ...
                    artifact_score_image = artifact_score_image.convert('RGBA').resize((artifact_score_image.width // 11, artifact_score_image.height // 11))
...
def main():
    ArtifacterFunc = Artifacter(input("UIDを入力してください: "))
    SCORE_STATE_LIST = ['HP', '攻撃', '防御', 'チャージ', '元素熟知']
    player_info = ArtifacterFunc.playerinfo()
    if 'showAvatarInfoList' in player_info:
        ...
        data = ArtifacterFunc.full_playerinfo(avatar_id)
        if len(data) > 0:
            ...
            ArtifacterFunc.create_buildcard(data, score_state)
